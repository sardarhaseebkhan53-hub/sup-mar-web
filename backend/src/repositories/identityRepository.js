...
class MemoryIdentityRepository {
  ...
  #findUser(predicate, { includePassword = false } = {}) {
  ...
  async listUsers({ search = '', status, role, limit = 25 } = {}) {
  ...
  async listSecurityEvents({ userId, limit = 50 } = {}) { return [...this.events.values()].filter((event) => !userId || event.userId === String(userId)).sort((a, b) => b.createdAt - a.createdAt).slice(0, limit).map(copy); }
...
class MongoIdentityRepository {
  ...
  listUsers({ search = '', status, role, limit = 25 } = {}) {
    const filter = {};
  ...
  revokeAllUserSessions(userId, reason, exceptSessionId = null) { const filter = { userId, revokedAt: null }; if (exceptSessionId) filter._id = { $ne: exceptSessionId }; return Session.updateMany(filter, { $set: { revokedAt: new Date(), revokeReason: reason } }); }
  ...
  listSecurityEvents({ userId, limit = 50 } = {}) { return SecurityEvent.find(userId ? { userId } : {}).sort({ createdAt: -1 }).limit(limit).lean(); }
...
export function resetIdentityRepository() { memoryRepository.reset(); }
