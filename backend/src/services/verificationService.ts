...
const tokenPurposes = new Set<string>([AUTH_PURPOSES.EMAIL_VERIFICATION, AUTH_PURPOSES.PASSWORD_RESET_EMAIL]);
...
export async function issueVerificationChallenge({ userId = null, target, purpose, channel, metadata = {}, isResend = false }: ChallengeInput) {
  ...
  if (previous && !previous.consumedAt && new Date(previous.resendAvailableAt) > now) {
    const retryAfter = Math.ceil((new Date(previous.resendAvailableAt).getTime() - now.getTime()) / 1000);
...
export async function verifyChallenge({ target, purpose, secret }: { target: string; purpose: string; secret: string }) {
  const repository = getIdentityRepository();
  ...
  if (hmacSecret(target, purpose, secret) !== challenge.secretHash) {
    ...
    const updates: { attempts: number; lockedUntil?: Date } = { attempts };
