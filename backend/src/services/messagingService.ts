...
export async function findOrCreateConversation(buyerId: string, listingKey: string) {
  ...
  const { assertNoRestriction } = await import('./trustSafetyService.js'); await assertNoRestriction(buyerId, ['ACCOUNT','MESSAGING']); await assertNoRestriction(sellerId, ['ACCOUNT','MESSAGING']);
  ...
  let record: any; let created = false;
  if (connected()) {
    record = await Conversation.findOne({ buyerId, sellerId, listingId: listing._id }).lean();
    if (!record) { const result: any = await Conversation.findOneAndUpdate({ buyerId, sellerId, listingId: listing._id }, { $setOnInsert: { buyerId, sellerId, listingId: listing._id } }, { new: true, upsert: true, includeResultMetadata: true }); record = result?.value?.toObject?.() || result?.value || result; created = result?.lastErrorObject?.updatedExisting === false || Boolean(result?.lastErrorObject?.upserted); }
  } else { record = [...conversations.values()].find((item) => item.buyerId === buyerId && item.sellerId === sellerId && item.listingPublicId === listing.publicId); if (!record) { const now = new Date(); record = { id: crypto.randomUUID(), buyerId, sellerId, listingId: listing.publicId, listingPublicId: listing.publicId, lastMessagePreview: '', unreadCountBuyer: 0, unreadCountSeller: 0, archivedBy: [], blockedBy: [], deletedBy: [], createdAt: now, updatedAt: now }; conversations.set(record.id, record); created = true; } }
  if (created) { const { incrementListingMessages } = await import('./listingService.js'); await incrementListingMessages(listing.publicId); }
...
export async function sendMessage(userId: string, id: string, input: { text?: string; attachments?: any[]; clientId: string }) { const conversation:any=connected()&&mongoose.isValidObjectId(id)?await Conversation.findById(id).lean():findMemoryConversation(id);assertMember(conversation,userId);const{assertNoRestriction}=await import('./trustSafetyService.js');await assertNoRestriction(userId,['ACCOUNT','MESSAGING']);if((conversation.blockedBy||[]).length)throw new AppError(403,'Messaging is blocked in this conversation','CONVERSATION_BLOCKED');
  const { areUsersBlocked } = await import('./blockService.js');
...
export async function adminConversationReports(){if(connected())return ConversationReport.find({}).sort({createdAt:-1}).lean();return[...reports.values()].sort((a,b)=>+b.createdAt-+a.createdAt)}export async function adminUpdateConversationReport(id:string,status:string,resolvedBy?:string){const resolution=['resolved','rejected'].includes(status)?{resolvedAt:new Date(),resolvedBy:resolvedBy||null}:{resolvedAt:null,resolvedBy:null};if(connected())return ConversationReport.findByIdAndUpdate(id,{$set:{status,...resolution}},{new:true}).lean();const item=reports.get(id);if(!item)throw new AppError(404,'Report not found','REPORT_NOT_FOUND');item.status=status;Object.assign(item,resolution);reports.set(id,item);return item}
export async function reportConversation(userId:string,id:string,input:any){const record:any=connected()&&mongoose.isValidObjectId(id)?await Conversation.findById(id).lean():findMemoryConversation(id);assertMember(record,userId);if(connected()){if(await ConversationReport.exists({conversationId:id,reporterId:userId,reason:input.reason,status:{$in:['pending','reviewed']}}))throw new AppError(409,'You already reported this conversation','REPORT_EXISTS');const report=await ConversationReport.create({conversationId:id,reporterId:userId,...input});return{id:String(report._id),status:report.status}}const duplicate=[...reports.values()].find((item)=>item.conversationId===id&&item.reporterId===userId&&item.reason===input.reason&&['pending','reviewed'].includes(item.status));if(duplicate)throw new AppError(409,'You already reported this conversation','REPORT_EXISTS');const report={id:crypto.randomUUID(),conversationId:id,reporterId:userId,...input,status:'pending',createdAt:new Date()};reports.set(report.id,report);return{id:report.id,status:report.status};}
...
export async function listNotifications(userId:string,limit=20){const {adminListAnnouncements}=await import('./adminAnnouncementService.js');await adminListAnnouncements({}).catch(()=>undefined);const rows:any[]=connected()?await Notification.find({userId}).sort({createdAt:-1}).limit(limit).lean():[...notifications.values()].filter((item)=>item.userId===userId).sort((a,b)=>+b.createdAt-+a.createdAt).slice(0,limit);return{notifications:rows.map(presentNotification),unread:rows.filter((item)=>!item.read).length};}
