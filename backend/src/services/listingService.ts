...
export async function createListing(userId: string, input: ListingInput) {
  ...
  if (!connected()) { memoryListings.set(id, payload); await syncAiArtifacts(payload, 'upsert'); return present(payload); }
  const created = present((await Listing.create(payload)).toObject());
  await syncAiArtifacts(created, 'upsert');
  return created;
}
...
export async function updateListing(userId: string, id: string, input: ListingInput) {
  ...
  if (!connected()) { const next = { ...current, ...patch }; memoryListings.set(current.publicId, next); await afterListingPriceChange(current, next); await syncAiArtifacts(next, 'upsert'); return present(next); }
  ...
  await syncAiArtifacts(saved, 'upsert');
  return saved;
}
...
export async function transitionListing(userId: string, id: string, action: 'publish' | 'pause' | 'resume' | 'sold' | 'remove') {
  ...
  await syncAiArtifacts(saved, ['removed', 'expired'].includes(saved.status) ? 'remove' : 'upsert');
...
/**
 * Phase 16: keep AI artefacts in sync with listing state.
 * Embeddings are refreshed off the write path and recommendation caches are
 * invalidated whenever a listing is sold, removed, expired or edited.
 */
async function syncAiArtifacts(record: any, event: 'upsert' | 'remove') {
  try {
    const [{ removeListingEmbedding, scheduleEmbeddingRefresh }, { invalidateRecommendationCache }] = await Promise.all([
      import('./listingEmbeddingService.js'),
      import('./recommendationService.js'),
    ]);
    if (event === 'remove' || !['published', 'sold', 'paused'].includes(record?.status)) await removeListingEmbedding(record?.publicId);
    else scheduleEmbeddingRefresh(record);
    invalidateRecommendationCache(`listing:${record?.publicId}:${event}`);
  } catch { /* AI sync must never break a listing write */ }
}
