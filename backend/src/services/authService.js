...
export async function registerWithEmail(input, req) {
  ...
  const user = await repository.createUser({
    name: input.name.trim(), username: await uniqueUsername(input.name), email, passwordHash,
    ...
    preferences: { language: input.language || 'en', notifications: { inApp: true, email: true, push: false, sms: true, security: true, marketing: false } },
