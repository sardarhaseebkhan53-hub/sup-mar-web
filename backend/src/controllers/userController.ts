...
import { completeAvatarUpload, createAvatarUploadIntent, removeAvatar } from '../services/avatarService.js';
import {
  changePassword,
...
export async function deleteAccount(req, res) { const data = await deleteAccountPermanently(req.auth.userId, req.body, req); clearRefreshCookie(res); res.json({ success: true, data }); }
...
export async function avatarRemove(req, res) { res.json({ success: true, data: await removeAvatar(req.auth.userId, req), message: 'Profile image removed' }); }
