...
export async function resendEmailVerification(req, res) { res.json({ success: true, data: await resendEmailVerificationChallenge(req.body.email, req) }); }
...
export async function verifyEmailLink(req, res) { res.json({ success: true, data: await verifyEmail({ email: req.query.target || req.query.email, token: req.query.token }, req), message: 'Email verified successfully' }); }
...
export async function authenticatedMe(req, res) { res.json({ success: true, data: await getProfile(req.auth.userId) }); }
