...
export async function userDetails(req, res) { const includeFinance=req.auth.roles.some((role:string)=>['admin','super_admin','finance'].includes(role));res.json({ success: true, data: await getUserForAdmin(req.params.id,includeFinance) }); }
