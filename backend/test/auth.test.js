...
const emailRegistration = (overrides = {}) => ({ method: 'email', name: 'Areeba Khan', email: 'areeba@example.com', password: strongPassword, confirmPassword: strongPassword, accountType: 'customer', country: 'PK', city: 'Rawalpindi', language: 'en', ...overrides });
const phoneRegistration = (overrides = {}) => ({ method: 'phone', name: 'Hamza Ali', phone: '03001234567', password: strongPassword, confirmPassword: strongPassword, accountType: 'seller', country: 'PK', city: 'Lahore', language: 'en', ...overrides });
...
describe('email registration and verification', () => {
  test('registers, rejects duplicates, verifies email, and signs in', async () => {
    const created = await request(app).post('/api/v1/auth/register').send(emailRegistration()).expect(201);
    ...
    const verified = await request(app).post('/api/v1/auth/verify-email').send({ email: 'areeba@example.com', token: delivery.secret }).expect(200);
