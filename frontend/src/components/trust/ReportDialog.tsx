...
export default function ReportSellerDialog({ open, onClose, targetId }: { open: boolean; onClose: () => void; targetId: string }) {
  ...
  return <Modal open={open} title="Report seller" description="Reports are reviewed by QAVLIO. This is not a public accusation." onClose={onClose}>
    {done ? <p className="text-sm font-semibold text-emerald-800">{done}</p> : <form onSubmit={(event) => { event.preventDefault(); submit.mutate(); }} className="space-y-3">
      <fieldset>
