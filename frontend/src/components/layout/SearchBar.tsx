...
export default function SearchBar({ variant = 'header', compact = false, className }: SearchBarProps) {
  ...
  if (isHero) {
    return <form onSubmit={handleSubmit} role="search" className={cn('grid gap-2 rounded-panel border border-ink-900/10 bg-white p-2 shadow-lg transition duration-200 focus-within:border-violet-300 focus-within:shadow-floating sm:grid-cols-[minmax(0,1fr)_180px_180px_auto]', className)}>
      <label className="relative flex min-w-0 items-center rounded-control bg-slate-50 sm:bg-transparent">
        ...
        <input value={query} onChange={(event) => setQuery(event.target.value)} type="search" placeholder='Search “iPhone, car, sofa…”' className="h-12 w-full min-w-0 bg-transparent pl-11 pr-3 text-sm font-semibold text-ink-900 outline-none placeholder:font-medium placeholder:text-slate-400" />
      </label>
  ...
  return <form onSubmit={handleSubmit} role="search" className={cn('flex w-full items-center rounded-control border border-ink-900/15 bg-white shadow-sm transition duration-200 focus-within:border-violet-400 focus-within:ring-4 focus-within:ring-violet-500/10', compact ? 'h-11' : 'h-12', className)}>
    <Search size={18} className="ml-3.5 shrink-0 text-violet-600" aria-hidden="true" />
    <label className="min-w-0 flex-1"><span className="sr-only">Search QAVLIO</span><input value={query} onChange={(event) => setQuery(event.target.value)} type="search" placeholder="Search products, cars, services…" className="h-full w-full min-w-0 border-0 bg-transparent px-3 text-sm font-semibold text-ink-900 outline-none placeholder:font-medium placeholder:text-slate-400" /></label>
