...
export default function ListingSection({ id, eyebrow, title, description, listings, variant, promoted = false }: ListingSectionProps) {
  return <section id={id} className={promoted ? 'border-y border-violet-100 bg-violet-50/70 py-12 sm:py-16' : 'pb-12 sm:pb-16'}>
    <div className="container-shell">
      <SectionHeading eyebrow={eyebrow} title={title} description={description} actionLabel="View all" actionTo={promoted ? '/marketplace?promoted=true' : '/marketplace?featured=true'} />
      {promoted && <p className="-mt-4 mb-5 inline-flex rounded-md bg-white px-2.5 py-1 text-[10px] font-bold text-slate-500 ring-1 ring-violet-200">Paid placements are always labeled “Sponsored”.</p>}
