...
export default function Hero() {
  ...
  const trustPoints = [
    { icon: BadgeCheck, label: t('home.trust.verified') },
    { icon: ShieldCheck, label: t('home.trust.safety') },
    { icon: MapPin, label: t('home.trust.local') },
  ];
      ...
      <div className="hero-grid relative overflow-hidden rounded-3xl border border-violet-200/70 bg-white shadow-card">
        ...
        <div className="grid min-h-[540px] items-center lg:grid-cols-[1.04fr_.96fr]">
          <div className="relative z-10 px-6 pb-7 pt-10 sm:px-10 lg:px-14 lg:py-14">
            <motion.span {...enter(0)} className="inline-flex items-center gap-2 rounded-full bg-violet-100 px-3 py-1.5 text-[11px] font-extrabold text-violet-700">
              <Sparkles size={14} /> {t('home.heroEyebrow')}
            </motion.span>
            <motion.h1 {...enter(0.08)} className="mt-6 max-w-xl text-display text-ink-950">
              {t('home.heroTitle').split(' ').map((word, i, arr) => (
                <span key={i}>
                  {i === arr.length - 1 ? <span className="text-violet-600">{word}</span> : `${word} `}
                </span>
              ))}
            </motion.h1>
            <motion.p {...enter(0.14)} className="mt-5 max-w-xl text-body-lg font-medium text-slate-600">
              {t('home.heroDescription')}
            </motion.p>
            <motion.div {...enter(0.2)} className="mt-7 flex flex-col gap-3 sm:flex-row">
              <Button to="/marketplace" size="lg">
                {t('home.exploreMarketplace')} <ArrowRight size={17} className="rtl-flip" />
              </Button>
              <Button to="/sell" size="lg" variant="secondary">
                <Plus size={17} /> {t('home.sellSomething')}
              </Button>
            </motion.div>
            <motion.div {...enter(0.26)} className="mt-8 flex flex-wrap gap-x-5 gap-y-2 border-t border-ink-900/10 pt-5">
              {trustPoints.map(({ icon: Icon, label }) => (
                <span key={label} className="inline-flex items-center gap-1.5 text-[11px] font-bold text-slate-500">
                  <Icon size={14} className="text-violet-600" /> {label}
                </span>
              ))}
            </motion.div>
            <div className="mt-6 grid grid-cols-3 gap-2 lg:hidden">
              <ImageWithFallback src={car960} srcSet={`${car480} 480w, ${car960} 960w`} alt="Car listing preview" loading="eager" wrapperClassName="aspect-[4/3] rounded-card" />
              <ImageWithFallback src={phone960} srcSet={`${phone480} 480w, ${phone960} 960w`} alt="Smartphone listing preview" loading="eager" wrapperClassName="aspect-[4/3] rounded-card" />
              <ImageWithFallback src={sofa960} srcSet={`${sofa480} 480w, ${sofa960} 960w`} alt="Sofa listing preview" loading="eager" wrapperClassName="aspect-[4/3] rounded-card" />
            </div>
          </div>

          <motion.div
            className="relative hidden min-h-[540px] lg:block"
            aria-label="A selection of products available across QAVLIO"
          ...
          >
            ...
            >
              <ImageWithFallback
                src={car960}
                srcSet={`${car480} 480w, ${car960} 960w`}
                alt="Silver car listing preview"
                loading="eager"
            ...
            <div className="absolute end-[7%] top-[15%] w-40 rotate-2 overflow-hidden rounded-card border-4 border-white bg-white shadow-floating">
              <ImageWithFallback
                ...
                alt="Smartphone listing preview"
                loading="eager"
                sizes="160px"
            ...
            <div className="absolute bottom-[7%] end-[3%] w-44 -rotate-2 overflow-hidden rounded-card border-4 border-white bg-white shadow-floating">
              <ImageWithFallback
                ...
                alt="Modern sofa listing preview"
                loading="eager"
                sizes="176px"
        ...
        <motion.div {...enter(0.3)} className="relative z-20 mx-4 mb-4 lg:-mt-8 lg:mx-10 lg:mb-8">
