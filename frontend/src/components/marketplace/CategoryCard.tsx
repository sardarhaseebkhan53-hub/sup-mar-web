...
export default function CategoryCard({ category }: CategoryCardProps) {
  ...
  return <motion.div whileHover={reduceMotion ? undefined : { y: -3 }} transition={{ duration: 0.2 }} className="min-w-0">
    <Link to={`/category/${category.slug}`} className="group flex min-h-[146px] min-w-[132px] flex-1 flex-col items-center justify-center rounded-card border border-ink-900/10 bg-white p-4 text-center shadow-sm transition duration-200 hover:border-violet-200 hover:shadow-card sm:min-w-0">
