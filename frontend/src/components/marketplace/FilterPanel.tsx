...
export default function FilterPanel({ params, dynamicFilters = [], onChange, onClear, onApply, mobile }: Props) {
  ...
  return <form onSubmit={(event) => { event.preventDefault(); onApply?.(); }} className={mobile ? 'bg-white' : 'rounded-2xl border border-ink-900/10 bg-white p-5 shadow-sm'} aria-label="Listing filters">
    ...
    <Group title="Date posted"><select value={params.get('date') || ''} onChange={(event) => onChange('date', event.target.value)} className="input-base !h-10 !px-3 !text-xs"><option value="">Any time</option><option value="today">Today</option><option value="3days">Last 3 days</option><option value="7days">Last 7 days</option><option value="30days">Last 30 days</option></select></Group>
    {dynamicFilters.filter((filter) => !['condition', 'listingType'].includes(filter.key)).map((filter) => <Group key={filter.key} title={filter.label}>{filter.type === 'boolean' ? <select value={params.get(`attr.${filter.key}`) || ''} onChange={(event) => onChange(`attr.${filter.key}`, event.target.value)} className="input-base !h-10 !text-xs"><option value="">Any</option><option value="true">Yes</option><option value="false">No</option></select> : filter.options ? <select value={params.get(`attr.${filter.key}`) || ''} onChange={(event) => onChange(`attr.${filter.key}`, event.target.value)} className="input-base !h-10 !text-xs"><option value="">Any</option>{filter.options.map((option) => <option key={option}>{option}</option>)}</select> : <input type="number" value={params.get(`attr.${filter.key}`) || ''} onChange={(event) => onChange(`attr.${filter.key}`, event.target.value)} className="input-base !h-10 !text-xs" />}</Group>)}
    {mobile && <button type="submit" className="mt-5 h-12 w-full rounded-control bg-violet-600 text-sm font-extrabold text-white">Show results</button>}
  </form>;
}
