...
interface LocationSelectorProps { value?: string; onChange?: (value: string) => void; compact?: boolean; className?: string; allowClear?: boolean; }
export function LocationSelector({ value, onChange, compact = false, className, allowClear = true }: LocationSelectorProps) {
  ...
  return <div className={cn('relative flex min-w-0 items-center', className)}>
    ...
    <select value={current || 'All Pakistan'} onChange={(event) => { const next = event.target.value; onChange?.(next); location.setCity(next); }} className={cn('w-full appearance-none bg-transparent pl-9 pr-5 font-bold text-ink-800 outline-none', compact ? 'h-10 text-xs' : 'h-12 text-sm')} aria-label="Location">
      {options.map((item) => <option key={item}>{item}</option>)}
    </select>
    <div className="absolute right-0 top-full z-20 hidden w-max pt-1 group-focus-within:block" />
    {allowClear && <div className="sr-only">
