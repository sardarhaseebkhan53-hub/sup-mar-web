...
export type ButtonVariant = 'primary' | 'secondary' | 'gold' | 'ghost' | 'danger' | 'success';
...
const variants: Record<ButtonVariant, string> = {
  ...
  success: 'bg-emerald-600 text-white shadow-sm hover:-translate-y-0.5 hover:bg-emerald-700 active:translate-y-0',
};
...
export function Button({ children, variant = 'primary', size = 'md', className, loading = false, disabled = false, type = 'button', to, state, onClick, ...props }: ButtonProps) {
  const classes = cn(
    'inline-flex items-center justify-center whitespace-nowrap font-extrabold transition duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-500 focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 motion-reduce:transform-none',
