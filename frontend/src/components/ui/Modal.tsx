...
export function Modal({ open, title, description, onClose, children }: ModalProps) {
  ...
  return <AnimatePresence>
    {open && <motion.div className="fixed inset-0 z-modal grid place-items-center bg-ink-950/55 p-4 backdrop-blur-sm" role="presentation" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
