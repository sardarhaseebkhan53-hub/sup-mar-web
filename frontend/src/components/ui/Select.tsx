...
export const Select = forwardRef<HTMLSelectElement, SelectProps>(function Select({ label, className, children, id, ...props }, ref) {
  ...
  return <label className="block" htmlFor={selectId}>
    ...
    <span className="relative block">
      <select ref={ref} id={selectId} className={cn('input-base appearance-none pe-10', className)} {...props}>{children}</select>
      <ChevronDown size={16} className="pointer-events-none absolute end-3.5 top-1/2 -translate-y-1/2 text-slate-400" aria-hidden="true" />
    </span>
  </label>;
});
