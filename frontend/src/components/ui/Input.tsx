...
export const Input = forwardRef<HTMLInputElement, InputProps>(function Input({ label, hint, error, leading, className, id, ...props }, ref) {
  ...
  return <label className="block" htmlFor={inputId}>
    ...
    <span className="relative block">
      {leading && <span className="pointer-events-none absolute start-3.5 top-1/2 -translate-y-1/2 text-slate-400">{leading}</span>}
      <input ref={ref} id={inputId} className={cn('input-base', Boolean(leading) && 'ps-10', error && 'border-rose-500 focus:border-rose-500 focus:ring-rose-500/10', className)} aria-invalid={Boolean(error)} aria-describedby={hint || error ? `${inputId}-description` : undefined} {...props} />
    </span>
