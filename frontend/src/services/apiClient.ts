...
export const trustApi = {
  ...
  myReports: () => apiRequest<any>('/reports'),
...
export const trustSafetyApi={
  verification:()=>apiRequest<any>('/verification/status'),submitVerification:(data:unknown)=>apiRequest<any>('/verification/submit',{method:'POST',body:json(data)}),documentIntent:(data:unknown)=>apiRequest<any>('/verification/documents/intent',{method:'POST',body:json(data)}),
  reports:()=>apiRequest<any[]>('/reports'),report:(data:unknown)=>apiRequest<any>('/reports',{method:'POST',body:json(data)}),
  appeals:()=>apiRequest<any[]>('/appeals'),submitAppeal:(data:unknown)=>apiRequest<any>('/appeals',{method:'POST',body:json(data)}),restrictions:()=>apiRequest<any[]>('/restrictions/me'),
  blockedUsers:()=>apiRequest<any[]>('/blocked-users'),blockedListings:()=>apiRequest<any[]>('/blocked-listings'),blockUser:(id:string)=>apiRequest<any>(`/users/${encodeURIComponent(id)}/block`,{method:'POST'}),unblockUser:(id:string)=>apiRequest<any>(`/users/${encodeURIComponent(id)}/block`,{method:'DELETE'}),blockListing:(id:string)=>apiRequest<any>(`/listings/${encodeURIComponent(id)}/block`,{method:'POST'}),unblockListing:(id:string)=>apiRequest<any>(`/listings/${encodeURIComponent(id)}/block`,{method:'DELETE'}),
};
export const adminTrustSafetyApi={
  verifications:(params='')=>apiRequest<any>(`/admin/verification${params?`?${params}`:''}`),verification:(id:string)=>apiRequest<any>(`/admin/verification/${id}`),verificationAction:(id:string,action:string,data:unknown)=>apiRequest<any>(`/admin/verification/${id}/${action}`,{method:'POST',body:json(data)}),
  appeals:(params='')=>apiRequest<any>(`/admin/appeals${params?`?${params}`:''}`),resolveAppeal:(id:string,data:unknown)=>apiRequest<any>(`/admin/appeals/${id}/resolve`,{method:'POST',body:json(data)}),
  moderation:()=>apiRequest<any>('/admin/moderation'),moderate:(id:string,data:unknown)=>apiRequest<any>(`/admin/moderation/${id}/action`,{method:'POST',body:json(data)}),
  restrictions:(params='')=>apiRequest<any[]>(`/admin/restrictions${params?`?${params}`:''}`),createRestriction:(data:unknown)=>apiRequest<any>('/admin/restrictions',{method:'POST',body:json(data)}),revokeRestriction:(id:string,reason:string)=>apiRequest<any>(`/admin/restrictions/${id}/revoke`,{method:'POST',body:json({reason})}),
  rules:()=>apiRequest<any[]>('/admin/moderation/rules'),createRule:(data:unknown)=>apiRequest<any>('/admin/moderation/rules',{method:'POST',body:json(data)}),updateRule:(id:string,data:unknown)=>apiRequest<any>(`/admin/moderation/rules/${id}`,{method:'PATCH',body:json(data)}),analytics:()=>apiRequest<any>('/admin/trust-safety/analytics'),violations:(userId:string)=>apiRequest<any[]>(`/admin/violations/${userId}`),
};
