...
export default function HelpPage() {
  ...
  return <div><Seo title="Help Center" description="Get help with your QAVLIO account, selling, payments, trust & safety, messages and support requests." canonicalPath="/help" /><section className="bg-ink-950 py-14 text-white sm:py-20"><div className="container-shell text-center"><span className="mx-auto grid h-12 w-12 place-items-center rounded-2xl bg-violet-600"><LifeBuoy /></span><p className="eyebrow mt-5 !text-gold-300">QAVLIO support</p><h1 className="mt-2 text-h1 text-white">How can we help?</h1><p className="mx-auto mt-3 max-w-xl text-sm text-white/55">Find clear answers, learn how to trade safely, or prepare a support request.</p><label className="relative mx-auto mt-7 block max-w-xl"><span className="sr-only">Search Help Center</span><Search className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" size={18} /><input value={query} onChange={(event) => setQuery(event.target.value)} className="h-12 w-full rounded-control bg-white pl-12 pr-4 text-sm text-ink-900 outline-none ring-offset-ink-950 focus:ring-2 focus:ring-violet-400" placeholder="Search help topics" /></label></div></section>
