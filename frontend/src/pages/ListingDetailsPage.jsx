...
export default function ListingDetailsPage() {
  ...
  const galleryImages = [listing, ...listings.filter((item) => item.id !== listing.id).slice(0, 3)];
  const [selectedImageIndex, setSelectedImageIndex] = React.useState(0);
  const [shareFeedback, setShareFeedback] = React.useState('Share');
  const heroImage = galleryImages[selectedImageIndex];
  async function shareListing() {
    const shareData = { title: listing.title, url: window.location.href };
    try {
      if (navigator.share) { await navigator.share(shareData); setShareFeedback('Shared'); return; }
      if (navigator.clipboard) { await navigator.clipboard.writeText(window.location.href); setShareFeedback('Link copied'); return; }
      setShareFeedback('Share unavailable');
    } catch { setShareFeedback('Share cancelled'); }
  }
      ...
      <div className="mt-4 grid gap-6 lg:grid-cols-[minmax(0,1fr)_370px]">
        <div className="min-w-0 space-y-5">
          <section className="overflow-hidden rounded-3xl border border-ink-900/10 bg-white shadow-sm">
            <div className="relative aspect-[16/10] overflow-hidden bg-slate-100 sm:aspect-[16/9]"><ImageWithFallback src={heroImage.image} srcSet={heroImage.imageSrcSet} sizes="(max-width: 1024px) 100vw, 70vw" alt={heroImage.imageAlt} wrapperClassName="h-full w-full" className="h-full w-full object-cover" /><div className="absolute left-4 top-4">{listing.featured && <Badge variant="featured">Featured listing</Badge>}</div><div className="absolute bottom-4 left-1/2 flex -translate-x-1/2 gap-1.5">{[0,1,2,3].map((value) => <span key={value} className={`h-1.5 rounded-full ${value === 0 ? 'w-5 bg-white' : 'w-1.5 bg-white/60'}`} />)}</div></div>
            <div className="grid grid-cols-4 gap-2 p-3">{galleryImages.map((item, index) => <button type="button" key={item.id} onClick={() => setSelectedImageIndex(index)} className={`aspect-[4/3] overflow-hidden rounded-xl border-2 ${index === selectedImageIndex ? 'border-violet-600' : 'border-transparent'}`} aria-label={`View image ${index + 1}`} aria-pressed={index === selectedImageIndex}><ImageWithFallback src={item.image} srcSet={item.imageSrcSet} sizes="25vw" alt={`${item.title} preview`} wrapperClassName="h-full w-full" className="h-full w-full object-cover" loading="lazy" /></button>)}</div>
          </section>
